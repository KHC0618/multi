...
/*배열은 Array타입 객체이며, Array.prototype의 프로퍼티를 상속받는다.
Array.prototype에는 수많은 메서드가 정의되어 있으며, 모든 배열은 이 메서드를 사용할 수 있다.
-> 이들 메서드를 활용하면, 직접 코드를 작성하지 않아도 다양한 배열을 처리할 수 있다.
*/
const arr = [];
// 배열의 생성자 함수는 Array
...
console.log(`foodList.indexOf('물회', 1) : ${foodList.indexOf('물회', 1)}`);            // 4 번쨰물회
...
// chineseFood.push('탕수육', '양장피');                         // 한 번에 push 가능
