...
SELECT SYSDATE, NEXT_DAY(SYSDATE,'금요일')NDAY FROM DUAL;
SELECT SYSDATE, NEXT_DAY(SYSDATE,6)NDAY FROM DUAL; -- 구하려는 요일이 숫자인경우 1 : 일요일~~~7 : 토요일
SELECT SYSDATE, NEXT_DAY(SYSDATE,'금')NDAY FROM DUAL;
...
SELECT ENAME 
	 ...
	 , CEIL(SYSDATE - HIREDATE) COL2
FROM EMP;

SELECT ENAME
	 , HIREDATE 
	 , LAST_DAY(HIREDATE) col1
	 , CEIL(LAST_DAY(HIREDATE) - HIREDATE) + 1 col2
FROM EMP;
...
SELECT ENAME, JOB, SAL,
	   DECODE(JOB, 'SALESMAN', SAL*1.1, 'MANAGER', SAL*1.15, 'ANALYST', SAL*1.2, SAL*1.05) AS 인상급여
FROM EMP;
