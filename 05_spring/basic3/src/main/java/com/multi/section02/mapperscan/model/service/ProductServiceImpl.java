...
//		this.sqlSession = sqlSession;
	...
	public List<ProductDTO> selectProductList() {
	 ...
	 /* 마이바티스 스캔을 사용하면 
	  * sqlsession 으로 부터 getMapper 로 ProductDAO.class 타입의 매퍼파일을 꺼내와서 매퍼객체의  
	  * selectProductList 결과로  리스트 형태로 반환 해준다
	  * MenuDAO 는 인터페이스고  구현체가 아니다. 그래서 의존주입시 @repository 가 정의된 빈이 없다. 그래서 에러발생. 
	  * 인터페이스로는 빈을 생성할수 없다. 
