...
-- INSERT INTO MEMBER VALUES('100', '100', 'park', '011', '1111');
-- SQL Error [913] [42000]: ORA-00913: too many values	지정된 값보다 더 많이 넣을 경우 발생하는 에러.

INSERT INTO MEMBER(ID, PW, NAME) VALUES('100', '100', 'park');

-- INSERT INTO MEMBER(ID, PW, NAME, COMPANY) VALUES('100', '100', 'park', 'LG');
-- SQL Error [904] [42000]: ORA-00904: "COMPANY": invalid identifier	정의되지 않은 컬럼에 값을 넣으려고 할 때 발생하는 에러

-- INSERT INTO MEMBER
-- 	(ID, PW, NAME, TEL)
-- VALUES
-- 	('', '', '');
-- SQL Error [947] [42000]: ORA-00947: not enough values	값이 부족할 때 발생하는 에러.
SELECT * FROM MEMBER;
COMMIT;
...
SELECT ID AS 아이디, NAME FROM "MEMBER" WHERE ID > 200;
...
-- 조회 시 없으면 에러가 아니라 0행을 출력.
SELECT ID, PW
FROM "MEMBER" 
WHERE ID = 'PAR';

-- 연결연산자를 이용해서 여러컬럼을 하나의 컬럼인 것처럼 연결할 수 있다 (||)
-- 컬럼과 컬럼을 연결

SELECT ID || ':' || NAME VAL FROM "MEMBER" ;

SELECT ID || '번의 이름은 ' || NAME || '입니다.' VAL FROM "MEMBER" ;
...
SELECT PW, NAME, ID FROM MEMBER ORDER BY 1, ID DESC; -- 오름차순 PW 기준
...
-- CHECK 제약조건 : 컬럼에 기록되는 값에 조건을 설정할수있음
-- CHECK(컬럼명 비교연산자 비교값 )
...
-- 컬럼 레벨일 경우
-- 컬럼명 자료형(크기) [CONSTRAINT 이름] REFERENCES 참조할테이블명 [(참조할컬럼)] [삭제rule]

-- 테이블 레벨일 경우
-- [CONSTRAINT 이름] FOREIGN KEY (적용할컬럼명) REFERENCES 참조할테이블명 [(참조할컬럼)] [삭제rule]
...
(
	...
	CONSTRAINT FK_REPLY_WRITER FOREIGN KEY (WRITER) REFERENCES MEMBER4(ID),
	CONSTRAINT FK_REPLY_ORIID FOREIGN KEY (ORIID) REFERENCES BBS4(NO)
);
...
INSERT INTO USER_FOREIGNKEY
    (USER_NO, USER_ID , USER_PWD , USER_NAME , GENDER , PHONE , EMAIL , GRADE_CODE)
VALUES
    (2, 'user02', 'pass02', '유재석', '남', '010-1234-5678', 'yu@naver.or.kr' ,10);
   
INSERT INTO USER_FOREIGNKEY
    (USER_NO, USER_ID , USER_PWD , USER_NAME , GENDER , PHONE , EMAIL , GRADE_CODE)
VALUES
    (3, 'user03', 'pass03', '유재석', '남', '010-1234-5678', 'yu@naver.or.kr' ,null);
   
INSERT INTO USER_FOREIGNKEY
    (USER_NO, USER_ID , USER_PWD , USER_NAME , GENDER , PHONE , EMAIL , GRADE_CODE)
VALUES
    (4, 'user04', 'pass04', '유재석', '남', '010-1234-5678', 'yu@naver.or.kr' ,50);
   
INSERT INTO USER_FOREIGNKEY
    (USER_NO, USER_ID , USER_PWD , USER_NAME , GENDER , PHONE , EMAIL , GRADE_CODE)
VALUES
    (5, 'user05', 'pass05', '유재석', '남', '010-1234-5678', 'yu@naver.or.kr' ,30);

COMMIT;

SELECT * FROM USER_FOREIGNKEY A
...
INSERT INTO TEST22 VALUES('111', '111', '122'); 
--SQL ERROR [2291] [23000]: ORA-02291: INTEGRITY CONSTRAINT (SCOTT.SYS_C007414) VIOLATED - PARENT KEY NOT FOUND
