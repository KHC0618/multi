...
FROM EMPLOYEES A 
LEFT JOIN DEPARTMENTS B ON A.DEPARTMENT_ID = B.DEPARTMENT_ID
WHERE (LOWER(A.FIRST_NAME) LIKE '%sa%') OR (LOWER(A.LAST_NAME) LIKE '%sa%');  
...
LEFT JOIN DEPARTMENTS B ON A.DEPARTMENT_ID = B.DEPARTMENT_ID
LEFT JOIN LOCATIONS C ON B.LOCATION_ID = C.LOCATION_ID
LEFT JOIN COUNTRIES D ON C.COUNTRY_ID = D.COUNTRY_ID
WHERE D.COUNTRY_NAME IN ('United States of America', 'Canada'); 
...
SELECT
	...
	B.FIRST_NAME || ' ' || B.LAST_NAME AS 동료이름
...
FROM EMPLOYEES A
LEFT JOIN JOBS B ON A.JOB_ID = B.JOB_ID
WHERE COMMISSION_PCT IS NULL AND B.JOB_ID IN ('FI_ACCOUNT', 'IT_PROG'); 
