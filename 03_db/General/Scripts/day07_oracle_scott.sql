...
-- 부서 별 급여의 합계가 가장 큰 부서의 부서명, 급여 합계를 구하세요.
...
SELECT DEPTNO , MAX(SYSDATE - HIREDATE) 
FROM EMP 
GROUP BY DEPTNO ;

SELECT *
FROM EMP 
WHERE (DEPTNO, SYSDATE - HIREDATE) IN (
SELECT DEPTNO , MAX(SYSDATE - HIREDATE) 
FROM EMP 
GROUP BY DEPTNO
);
...
--다중행다중열 서브쿼리를 이용 - 보통 다중행 다중열일가능성이 크다 
...
SELECT ROWNUM, A.DEPTNO , A.AVGSAL 
FROM (
